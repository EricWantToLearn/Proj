...
<body data-spy="scroll" data-target=".site-navbar-target" data-offset="200">
  ...
  <div class="site-wrap">
    ...
    <header class="header-bar d-flex d-lg-block align-items-center site-navbar-target" data-aos="fade-right">
      ...
      <div class="main-menu">
        ...
        <ul class="social js-clone-nav">
          ...
          <li><a href="https://www.tiktok.com/@dos.studiosph?_t=8oTvnnSYNco&_r=1" target="_blank"><i class="fab fa-tiktok"></i></a></li>
        </ul>
      </div>
    </header> 
    <main class="main-content">

      <section class="site-section-hero bg-image mb-5" style="background-image: url('images/background.png');"  data-stellar-background-ratio="0.5" id="section-home">
    ...
    <h1 class="heading">Book Now</h1></center> -->
            ...
            <p data-aos="fade-up" data-aos-delay="100"><a href="#section-contact" class="btn btn-primary btn-md smoothscroll">Book Now</a></p>
          </div>
        </div>
      </section>
      ...
      <div class="container-fluid site-section">
        ...
        <div class="row justify-content-center">
          <div class="col-md-12 text-center py-5">
            <p>Copyright &copy;<script>document.write(new Date().getFullYear());</script>. All Rights Reserved. &mdash; Designed with love by Optisnap</a>
            </p>
          </div>
        </div>
      </div>
...
function updatePackageList() {
    const packages = document.querySelectorAll('#packages .blog-entry');
    let packageList = '';

    packages.forEach(pkg => {
        const packageName = pkg.querySelector('h2').getAttribute('data-package-name');
        const quantityInput = pkg.querySelector('input[type="number"]');
        const quantity = parseInt(quantityInput.value);
        
        if (quantity > 0) {
            packageList += packageName + ' x ' + quantity + ' | '; // Concatenate if quantity is more than zero
        }
    });

    packageList = packageList.slice(0, -3); // Remove the last ' | '.
    document.getElementById('hiddenpackagelist').value = packageList; // Store in the hidden input
}
    // Automatically call updateHiddenPackageList on page load
    updateHiddenPackageList();
